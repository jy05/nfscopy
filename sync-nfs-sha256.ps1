...
$workerScript = {
    ...
    function New-Line {  # "<timestamp>  [STAGE]  <relpath>  SHA256=<hash>"
        param([string]$Stage, [string]$Rel, [string]$Hash)
        "{0:yyyy-MM-dd HH:mm:ss}  [{1,-10}] {2}  SHA256={3}" -f (Get-Date), $Stage, $Rel, $Hash
    }
    ...
    try {
        ...
        $incomingHash = (Get-FileHash -LiteralPath $Src.FullName -Algorithm SHA256).Hash
        ...
        $procHash = (Get-FileHash -LiteralPath $proc -Algorithm SHA256).Hash
        $block += New-Line -Stage 'PROCESSING' -Rel $rel -Hash $procHash
        if ($procHash -ne $incomingHash) {
            throw "SHA256 mismatch incoming=$incomingHash processing=$procHash"
        }
        ...
        $mainHash = (Get-FileHash -LiteralPath $dest -Algorithm SHA256).Hash
